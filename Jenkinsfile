        ...
        stage('Add Step to EMR Cluster') {
            steps {
                script {
                   ...
                   def stepId = sh(script: "aws emr list-steps --cluster-id ${env.CLUSTER_ID} --region ${REGION} --query --query "Steps | sort_by(@, &CreationDate) | [-1].Id" --output text", returnStdout: true).trim()
