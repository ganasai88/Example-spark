        ...
        stage('Add Step to EMR Cluster') {
            steps {
                script {
                   ...
                   def stepId = sh(script: "aws emr list-steps --cluster-id ${env.CLUSTER_ID} --region ${REGION} --query 'Steps | sort_by(@, &Timeline.CreationDateTime) | [-1].Id' --output text", returnStdout: true).trim()
